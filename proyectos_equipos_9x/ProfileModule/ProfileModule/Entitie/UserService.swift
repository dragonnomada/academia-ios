...
class UserAuthService {
    ...
    func LogInUser(email: String, password: String) {
        ...
        if let userLogged = allUsers.filter({
            ...
            NotificationCenter.default.post(name: NSNotification.Name("User.Auth.Logged"), object: userLogged)
            
        } else {
            
            NotificationCenter.default.post(name: NSNotification.Name("User.Auth.Logged"), object: UserAuthServiceErrors.loginUser(reason: "User Loggin Failed, not user Register, not the right password or email"))
